...
class ChessBoard:
    def __init__(self, screen, width, height):
        ...
        self.pawn_updater_position = (
            width - self.box_size - 10,
            height // 2 - 2 * self.box_size,
        )
        ...
        self.b_dead_piece_surf = pygame.surface.Surface(
            (2 * self.box_size, 8 * self.box_size), pygame.SRCALPHA
        )
        self.w_dead_piece_surf = pygame.surface.Surface(
            (2 * self.box_size, 8 * self.box_size), pygame.SRCALPHA
        )
    ...
    def _update_black_attack_positions(self):
        ...
        # REMOVED ALL REPETITIVE POSITIONS
        points = list(set(points))
    ...
    def resized(self, width, height):
        ...
        # SURFACE ALIGNMENT OVER THE MAIN SCREEN
        self.board_surf_rect = self.board_surf.get_rect()
        self.board_surf_rect.center = self.screen.get_rect().center
        self.b_dead_piece_surf_rect = self.b_dead_piece_surf.get_rect()
        self.b_dead_piece_surf_rect.right = self.board_surf_rect.left
    ...
    def draw_board(self):
        ...
        pygame.draw.rect(
            self.board_surf, self.secondary_color, (0, 0, self.size, self.size)
        )
        pygame.draw.rect(
            self.board_surf, self.primary_color, (3, 3, self.size - 6, self.size - 6)
        )
        flag = 1
        ...
        selector_layer = self._highlight_boxes(
            self.valid_moves, self.pieces_under_attack
        )
        ...
        for i in range(8):
            for j in range(8):
                self._get_piece(
                    ...
                    self.board_surf,
                    (
                        16 + j * self.box_size + self.box_center,
                        16 + i * self.box_size + self.box_center,
                    ),
                )
        ...
        pygame.draw.rect(
            self.board_surf,
            self.secondary_color,
            (16, 16, self.size - 32, self.size - 32),
            width=2,
        )
    ...
    def _display_dead_piece(self):
        ...
        for piece in self.b_dead_pieces:
            self._get_piece(
                piece,
                self.b_dead_piece_surf,
                (
                    i * self.box_size + self.box_center,
                    j * self.box_size + self.box_center,
                ),
            )
    ...
    def _pawn_update_selector(self, player):
        ...
        inactive_layer = pygame.surface.Surface(
        ...
        for i in range(4):
            pygame.draw.rect(
                local_surface,
                (0, 0, 0),
                (0, i * self.box_size, self.box_size, self.box_size + 1),
                1,
            )

        if player == "white":
            local_surface.blit(
                self.piece.get_piece("white queen"),
                (0 + offset, 0 + self.box_size * 0 + offset),
            )
            local_surface.blit(
                self.piece.get_piece("white rook"),
                (0 + offset, 0 + self.box_size * 1 + offset),
            )
            local_surface.blit(
                self.piece.get_piece("white bishop"),
                (0 + offset, 0 + self.box_size * 2 + offset),
            )
            local_surface.blit(
                self.piece.get_piece("white knight"),
                (0 + offset, 0 + self.box_size * 3 + offset),
            )
        else:
            local_surface.blit(
                self.piece.get_piece("black queen"),
                (0 + offset, 0 + self.box_size * 0 + offset),
            )
            local_surface.blit(
                self.piece.get_piece("black rook"),
                (0 + offset, 0 + self.box_size * 1 + offset),
            )
            local_surface.blit(
                self.piece.get_piece("black bishop"),
                (0 + offset, 0 + self.box_size * 2 + offset),
            )
            local_surface.blit(
                self.piece.get_piece("black knight"),
                (0 + offset, 0 + self.box_size * 3 + offset),
            )
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 1:
                        if event.pos[0] in range(x_min, x_min + self.box_size):
                            if event.pos[1] in range(
                                y_min + self.box_size * 0, y_min + self.box_size * 1
                            ):
                                selected_piece = 1
                            elif event.pos[1] in range(
                                y_min + self.box_size * 1, y_min + self.box_size * 2
                            ):
                                selected_piece = 2
                            elif event.pos[1] in range(
                                y_min + self.box_size * 2, y_min + self.box_size * 3
                            ):
                                selected_piece = 3
                            elif event.pos[1] in range(
                                y_min + self.box_size * 3, y_min + self.box_size * 4
                            ):
                                selected_piece = 4
    ...
    def _box_over_layer(self, layer_surface, i, j, color, i_offset=0, j_offset=0):
        ...
        for k in range(255, -1, -255 // (self.box_size // 3)):
            pygame.draw.rect(
                ...
                color + [k],
                (
                    j_offset + j * self.box_size + d,
                    i_offset + i * self.box_size + d,
                    self.box_size - 2 * d,
                    self.box_size - 2 * d,
                ),
                1,
    ...
    def _highlight_boxes(self, yellow_boxes, red_boxes):
        ...
        layer_surface = pygame.surface.Surface(
            (int(self.box_size * 8), int(self.box_size * 8)), pygame.SRCALPHA
        )
    ...
    def king_move(self, player, move_from, move_to):
        valid_moves, killing_moves, garbage = self.get_king_moves(
            player, move_from[0], move_from[1]
        )
    ...
    def knight_move(self, player, move_from, move_to):
        valid_moves, killing_moves, garbage = self.get_knight_moves(
            player, move_from[0], move_from[1]
        )
        return self.movement_helper(
            player, valid_moves, killing_moves, move_from, move_to
        )

    def rook_move(self, player, move_from, move_to):
        valid_moves, killing_moves, garbage = self.get_rook_moves(
            player, move_from[0], move_from[1]
        )
        return self.movement_helper(
            player, valid_moves, killing_moves, move_from, move_to
    ...
    def bishop_move(self, player, move_from, move_to):
        valid_moves, killing_moves, garbage = self.get_bishop_moves(
            player, move_from[0], move_from[1]
        )
        return self.movement_helper(
            player, valid_moves, killing_moves, move_from, move_to
    ...
    def queen_move(self, player, move_from, move_to):
        return self.rook_move(player, move_from, move_to) or self.bishop_move(
            player, move_from, move_to
        )
    ...
    def pawn_move(self, player, move_from, move_to):
        ...
        if dist == 1:
            if self.grid[move_to[0]][move_to[1]] == 0:
                if (move_to[0] - move_from[0] == 1 and player == "black") or (
                    move_to[0] - move_from[0] == -1 and player == "white"
                ):
                    temp_loc = move_to
                    self.grid[move_to[0]][move_to[1]] = self.grid[move_from[0]][
                        move_from[1]
                    ]
                    self.grid[move_from[0]][move_from[1]] = 0
                    if player == "black" and temp_loc[0] == 7:
                        # SHOW BLACK PIECES SELECTOR
                        self.grid[temp_loc[0]][temp_loc[1]] = (
                            self._pawn_update_selector("black")
                        )
                    elif player == "white" and temp_loc[0] == 0:
                        # SHOW WHITE PIECES SELECTOR
                        self.grid[temp_loc[0]][temp_loc[1]] = (
                            self._pawn_update_selector("white")
                        )
                    return True

        elif dist == 2:
            if (
                self.grid[move_to[0]][move_to[1]] != 0
                and self.grid[move_to[0]][move_to[1]][:5] != player
            ):
                if player == "white":
                    self.b_dead_pieces.append(self.grid[move_to[0]][move_to[1]])
                else:
                ...
                if player == "black" and temp_loc[0] == 7:
                    # SHOW BLACK PIECES SELECTOR
                    self.grid[temp_loc[0]][temp_loc[1]] = self._pawn_update_selector(
                        "black"
                    )
                elif player == "white" and temp_loc[0] == 0:
                    # SHOW WHITE PIECES SELECTOR
            ...
            if (
                abs(move_to[0] - move_from[0]) == 2
                and self.grid[move_to[0]][move_to[1]] == 0
            ):
                if (
                    player == "white"
                    and move_from[0] == 6
                    and self.grid[move_from[0] - 1][move_from[1]] == 0
                ) or (
                    player == "black"
                    and move_from[0] == 1
                    and self.grid[move_from[0] + 1][move_from[1]] == 0
    ...
    def select_box(self, pos, selector, player):
        ...
        if x != None and y != None:
            if selector == 1:
                self.move_from = [x, y]
                self.move_to = []
                self.valid_moves, self.pieces_under_attack, garbage = (
                    self.get_valid_moves(name[player], self.move_from)
                )
            else:
                self.move_to = [x, y]
    ...
    def get_king_moves(self, player, i, j):
        ...
        for point in temp_points:
            if point[0] in range(0, 8) and point[1] in range(0, 8):
                if (
                    player == "black" and self.white_attacking_cells[point[0]][point[1]]
                ) or (
                    player == "white" and self.black_attacking_cells[point[0]][point[1]]
                ):
                    pass
                else:
                    if self.grid[point[0]][point[1]] == 0:
        ...
        if player == "white":
            if self.white_castle:
                if [i, j] == [7, 4]:
                    if (
                        self.grid[7][0] == "white rook"
                        and self.grid[7][1:4] == [0, 0, 0]
                        and not self.black_attacking_cells[7][2]
                    ):
                        moves.append((7, 2))
                    elif (
                        self.grid[7][7] == "white rook"
                        and self.grid[7][5:7] == [0, 0]
                        and not self.black_attacking_cells[7][6]
                    ):
                        moves.append((7, 6))
        else:
            if self.black_castle:
                if [i, j] == [0, 4]:
                    if (
                        self.grid[0][0] == "black rook"
                        and self.grid[0][1:4] == [0, 0, 0]
                        and not self.white_attacking_cells[0][2]
                    ):
                        moves.append((0, 2))
                    elif (
                        self.grid[0][7] == "black rook"
                        and self.grid[0][5:7] == [0, 0]
                        and not self.white_attacking_cells[0][6]
                    ):
                        moves.append((0, 6))
        return moves, under_attack_moves, self_attack_moves
